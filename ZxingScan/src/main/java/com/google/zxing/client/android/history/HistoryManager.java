...
public final class HistoryManager {
    ...
    public HistoryManager(Context activity) {
...
//    enableHistory = prefs.getBoolean(PreferencesActivity.KEY_ENABLE_HISTORY, true);
    ...
    public List<BarcodeResult> getHistoryItems() {
        SQLiteOpenHelper helper = new DBHelper(activity);
        List<BarcodeResult> items = new ArrayList<>();
        SQLiteDatabase db = null;
        Cursor cursor = null;
        try {
            db = helper.getReadableDatabase();
            cursor = db.query(DBHelper.TABLE_NAME, COLUMNS, null, null, null, null, DBHelper.TIMESTAMP_COL + " DESC limit 20");
            while (cursor.moveToNext()) {
                ...
                items.add(new BarcodeResult(text, format, display, timestamp, type));
            }
        } finally {
            close(cursor, db);
        }
    ...
    public void addHistoryItem(BarcodeResult result) {
        if (result == null) return;
        deletePrevious(result.text);
        ContentValues values = new ContentValues();
        values.put(DBHelper.BARCODE_TYPE, result.type);
        values.put(DBHelper.TEXT_COL, result.text);
        values.put(DBHelper.FORMAT_COL, result.format);
        values.put(DBHelper.DISPLAY_COL, result.display);
